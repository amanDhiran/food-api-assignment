...
export default function ProductList() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      ...
      setCategories(fetchedCategories.sort((a: Category, b: Category) => 
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {loading ? <FoodProductsSkeleton/> :
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map((product, i) => (
          product.product_name && <Link href={{
            pathname: `/product/${product.code}`,
          }} key={i} className="border rounded-lg p-4 shadow-md">
            ...
            <p className="text-gray-600 mb-2"> <span className='font-semibold mr-2'>Category:</span> 
            {filterAndRemoveEnglishPrefix(product.categories_tags).join(', ')
            }
            </p>
            <p className="text-sm text-gray-500 mb-2">Barcode: {Number(product.code)}</p>
            <p className="text-sm text-gray-500 mb-2">Ingredients: {filterAndRemoveEnglishPrefix(product.ingredients_tags).join(', ')}</p>
      ...
      {products.length < 20 ? "" : (
        ...
        {fetchingMoreProducts ?<Loader2 className="h-8 w-8 animate-spin text-primary"/> : <Button variant={'outline'} onClick={loadMoreProducts} className="mt-6">
          Load More...
        </Button>}
        </div> 
      )}
...
export function FoodProductsSkeleton() {
  return (
